...
import { 
  FaStore, FaBoxes, FaClipboardList, FaTruck, FaChartLine, 
  FaSignOutAlt, FaEdit, FaPlus, FaTrash, FaCheck, FaTimes,
  FaUserCog, FaBell, FaSearch, FaQrcode, FaFileExport, 
  FaShoppingCart, FaStar, FaComments
} from 'react-icons/fa';
...
const ShopkeeperDashboard = () => {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, message: "New order received #104", time: "2 mins ago", read: false },
    { id: 2, message: "Product 'Organic Apples' is low in stock", time: "1 hour ago", read: false },
    { id: 3, message: "Monthly sales report is ready", time: "1 day ago", read: true }
  ]);
  ...
  const [products, setProducts] = useState([
    { id: 1, name: "Organic Apples", price: 120, stock: 45, image: "https://images.unsplash.com/photo-1568702846914-96b305d2aaeb", category: "Fruits" },
    { id: 2, name: "Fresh Milk 1L", price: 55, stock: 32, image: "https://images.unsplash.com/photo-1550583724-b2692b85b150", category: "Dairy" },
    { id: 3, name: "Whole Wheat Bread", price: 35, stock: 18, image: "https://images.unsplash.com/photo-1509440159596-0249088772ff", category: "Bakery" },
    { id: 4, name: "Free Range Eggs (Dozen)", price: 60, stock: 24, image: "https://images.unsplash.com/photo-1587486913049-53fc88980bea", category: "Dairy" }
  ]);

  const [orders, setOrders] = useState([
    { id: 101, customer: "Rahul Sharma", items: "Apples, Milk", total: 175, status: "Processing", date: "2023-06-15" },
    { id: 102, customer: "Priya Patel", items: "Bread, Eggs", total: 95, status: "Shipped", date: "2023-06-14" },
    { id: 103, customer: "Amit Singh", items: "Apples, Bread", total: 155, status: "Delivered", date: "2023-06-13" },
    { id: 104, customer: "Neha Gupta", items: "Milk, Eggs", total: 115, status: "Processing", date: "2023-06-15" }
  ]);

  const [reviews, setReviews] = useState([
    { id: 1, customer: "Rahul Sharma", rating: 5, comment: "Excellent quality products!", date: "2023-06-10" },
    { id: 2, customer: "Priya Patel", rating: 4, comment: "Fast delivery, fresh items", date: "2023-06-08" },
    { id: 3, customer: "Amit Singh", rating: 3, comment: "Good but some items were missing", date: "2023-06-05" }
  ]);
                ...
                <AnimatePresence>
                  {showNotifications && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -20 }}
                      className="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-xl z-50"
                    >
        ...
        >
          {[
            ...
            <button
              ...
              className={`flex items-center px-6 py-4 font-medium text-sm md:text-base transition-all ${activeTab === tab.id ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    {editMode ? (
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">{shopData.name}</div>
                    )}
                  </div>
                  <div>
                    ...
                    {editMode ? (
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">{shopData.category}</div>
                    )}
                  </div>
                  <div className="md:col-span-2">
                    ...
                    {editMode ? (
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">{shopData.description}</div>
                    )}
                  </div>
                  <div className="md:col-span-2">
                    ...
                    {editMode ? (
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">{shopData.address}</div>
                    )}
                  </div>
                  <div>
                    ...
                    {editMode ? (
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">{shopData.phone}</div>
                    )}
                  </div>
                  <div>
                    ...
                    {editMode ? (
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">{shopData.email}</div>
                    )}
                  </div>
                  <div>
                    ...
                    {editMode ? (
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">{shopData.openingHours}</div>
                    )}
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-gray-700 mb-2">QR Code for Customers</label>
                    <div className="p-4 bg-gray-50 rounded-lg flex items-center justify-center">
                      <div className="text-center">
                        <FaQrcode className="text-6xl mx-auto text-gray-400 mb-2" />
                        <p className="text-sm text-gray-500">Scan to visit your store</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                  ...
                  <div className="flex space-x-3">
                    ...
                    <button 
                      className="flex items-center bg-white border border-green-600 text-green-600 hover:bg-green-50 px-4 py-2 rounded-lg"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredOrders.map(order => (
                    ...
                    >
                      ...
                      <div className="p-6">
                        <p className="mb-3"><span className="font-medium">Items:</span> {order.items}</p>
                        <p className="mb-4"><span className="font-medium">Total:</span> ₹{order.total}</p>
                        
                        <div className="flex flex-wrap gap-2">
                          <button
                            onClick={() => updateOrderStatus(order.id, 'Processing')}
                            className={`px-3 py-1 rounded-lg text-xs ${order.status === 'Processing' ? 'bg-yellow-600 text-white' : 'bg-yellow-100 text-yellow-800'}`}
                          >
                            Processing
                          </button>
                          <button
                            onClick={() => updateOrderStatus(order.id, 'Shipped')}
                            className={`px-3 py-1 rounded-lg text-xs ${order.status === 'Shipped' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-800'}`}
                          >
                            Shipped
                          </button>
                          <button
                            onClick={() => updateOrderStatus(order.id, 'Delivered')}
                            className={`px-3 py-1 rounded-lg text-xs ${order.status === 'Delivered' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-800'}`}
                          >
                            Delivered
                          </button>
                        </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                  ...
                  >
                    ...
                    <p className="text-3xl font-bold mt-2">₹{stats.todaySales.toLocaleString()}</p>
                  ...
                  >
                    ...
                    <p className="text-3xl font-bold mt-2">₹{stats.monthlySales.toLocaleString()}</p>
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-500 mt-1">+15 new this week</p>
                  </motion.div>
                  <motion.div
                    whileHover={{ y: -5 }}
                  ...
                  >
                    ...
                    <div className="flex items-center mt-2">
                      <div className="text-3xl font-bold mr-2">{stats.averageRating}</div>
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map(star => (
                          <FaStar 
                            key={star}
                            className={`text-lg ${star <= Math.round(stats.averageRating) ? 'text-yellow-500' : 'text-gray-300'}`}
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {products.slice(0, 4).map((product, index) => (
                        <div key={product.id} className="flex items-center">
                          ...
                          <img src={product.image} alt={product.name} className="w-10 h-10 rounded-full object-cover mr-3" />
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
              ...
              <p className="text-gray-400">Your neighborhood grocery store providing fresh products daily.</p>
            </div>
            <div>
          ...
          <div className="border-t border-gray-700 mt-8 pt-6 text-center text-gray-400">
            <p>© {new Date().getFullYear()} FreshMart Groceries. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
